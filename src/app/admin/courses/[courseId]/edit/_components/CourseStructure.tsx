...
export default function CourseStructure({ data }: iAppProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (!targetChapterId) {
        toast.error("Couuldnt determine target chapter");
        return;
      }
      ...
      if (oldIndex === -1 || newIndex === -1) {
        toast.error("Couuldnt find chapter old/new index for reordering");
        return;
      }

      // create new array with elements moved to new position
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (chapterIndex === -1) {
        toast.error("Couuldnt find chapter index for reordering");
        return;
      }
      ...
      if (oldLessonIndex === -1 || newLessonIndex === -1) {
        toast.error("Couuldnt find lesson for reordering");
        return;
      }
...
function SortableItem({ children, id, className, data }: SoratbleItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
